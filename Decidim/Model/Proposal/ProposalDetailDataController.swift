...
class ProposalDetailDataController: NetworkDataController {
    ...
    override func fetchPage(cursor: NetworkDataController.Cursor, completion: @escaping ([Any]?, NetworkDataController.Cursor?, Error?) -> Void) {
        ...
        let testDetail = ProposalDetail(proposal: self.backingProposal, myVote: nil, author: "Test Author \(backingProposal.id + 1)", likeCount: randomInt % 200, deadline: Date(timeIntervalSinceNow: TimeInterval(randomInt % 50000)), amendmentCount: randomInt % 24, comments: testComments)
